...
impl<'a, 'gcc, 'tcx> IntrinsicCallBuilderMethods<'tcx> for Builder<'a, 'gcc, 'tcx> {
    fn codegen_intrinsic_call(
        ...
        let value = match name {
            ...
            {
                let func = get_simple_function_f128(self, name);
                self.cx.context.new_call(
                    self.location,
                    func,
                    &args.iter().map(|arg| arg.immediate()).collect::<Vec<_>>(),
            ...
            sym::maxnumf128 | sym::minnumf128 | sym::copysignf128 if self.cx.supports_f128_type => {
                let func = get_simple_function_f128_2args(self, name);
                self.cx.context.new_call(
                    self.location,
                    func,
                    &args.iter().map(|arg| arg.immediate()).collect::<Vec<_>>(),
            ...
            sym::fabs => 'fabs: {
                ...
                let func = match float_ty {
                    ...
                    ty::FloatTy::F128 => get_simple_function_f128(self, name),
                };
                self.cx.context.new_call(
                    self.location,
                    func,
